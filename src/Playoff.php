<?php
  namespace Playoff\Sdk;

  class PlayoffException extends \Exception {
  ...
  class Playoff {
    ...
    function __construct(array $params) {
      if (!extension_loaded('curl')) {
        throw new Exception('The PHP exention curl must be installed to use this library.', PlayoffException::CURL_NOT_FOUND);
      }
      ...
      if(array_key_exists('domain', $params)) {
        $this->authorization_endpoint = "https://" . $params['domain'] . "/auth/token";
        $this->api_endpoint = "https://api." . $params['domain'] . "/";
        $this->domain = $params['domain'];
      }
      else {
        $this->authorization_endpoint = "https://playoffgamification.io/auth/token";
        $this->api_endpoint = "https://api.playoffgamification.io/";
        $this->domain = "playoffgamification.io"; 
      }
      if(array_key_exists('type', $params)) {
        $this->type = $params['type'];
      }
      else {
        throw new PlayoffException('init_failed', "You must pass in a type whether 'client' for client credentials flow or 'code' for auth code flow");
      }
      ...
      if ($this->type == 'code') {
        if(array_key_exists('redirect_uri', $params)) {
          ...
          throw new PlayoffException('init_failed', "You must pass in a redirect_uri for the auth code flow");
        }
      }
    }
    ...
    private function get_access_token() {
      if($this->type == 'client') {
        ...
        $access_token = $this->executeRequest('POST', $this->authorization_endpoint, null, $data);
      }
      else {
        $data = array(
          'client_id' => $this->client_id,
    ...
    public function api($http_method = self::HTTP_METHOD_GET, $route , $query = array(), $body = array(), $raw = false) {
      ...
      return $this->executeRequest($http_method, $this->api_endpoint.$this->version.$route, $query, $body, $raw);
    }

    public function get($route, $query = array(), $raw = false) {
      $this->check_token($query);
      return $this->executeRequest(self::HTTP_METHOD_GET, $this->api_endpoint.$this->version.$route, $query, null, $raw);
    }
    ...
    public function post($route, $query = array(), $body = array()) {
      ...
      return $this->executeRequest(self::HTTP_METHOD_POST, $this->api_endpoint.$this->version.$route, $query, $body);
    }

    public function patch($route, $query = array(), $body = array()) {
      $this->check_token($query);
      return $this->executeRequest(self::HTTP_METHOD_PATCH, $this->api_endpoint.$this->version.$route, $query, $body);
    }

    public function put($route, $query = array(), $body = array()) {
      $this->check_token($query);
      return $this->executeRequest(self::HTTP_METHOD_PUT, $this->api_endpoint.$this->version.$route, $query, $body);
    }

    public function delete($route, $query = array()) {
      $this->check_token($query);
      return $this->executeRequest(self::HTTP_METHOD_DELETE, $this->api_endpoint.$this->version.$route, $query);
    }
    ...
    private function executeRequest($http_method = self::HTTP_METHOD_GET, $url, $query = array(), $body = array(), $raw = false)
    {
        ...
        if ($curl_error = curl_error($ch)) {
          throw new \Exception($curl_error, PlayoffException::CURL_ERROR);
        } else {
          if($raw === true){
            return $result;
          }
        ...
        if(array_key_exists('error', $json_decode)) {
          throw new PlayoffException($json_decode['error'], $json_decode['error_description']);
        }
        else {
          #print_r($json_decode);
          return $json_decode;
    ...
    public function exchange_code($code) {
      if($code == null) {
        throw new PlayoffException('invalid_request', "You must pass in a code in exchange_code for the auth code flow");
      }
      $this->code = $code;
      $this->get_access_token();
    }
    ...
    public function get_login_url() {
      ...
      return "https://" . $this->domain . "/auth?" . http_build_query($query, null, '&');
    }
    ...
    public function upload_image($file) {
      $query = array();
      $this->check_token($query);
      $url = $this->api_endpoint.$this->version.'/design/images?' . http_build_query($query, null, '&');
      $ch = curl_init();
    ...
    public function read_image($image_id, $query = array()) {
      $this->check_token($query);
      $url = $this->api_endpoint.$this->version.'/design/images/'.$image_id.'?' . http_build_query($query, null, '&');
      $ch = curl_init();
      curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
      curl_setopt($ch, CURLOPT_URL, $url);
      curl_setopt($ch, CURLOPT_HEADER, 0);
    ...
    public static function createJWT(array $params) {
      ...
      $segments = array();
      $segments[] = Playoff::urlsafeB64Encode(json_encode($header));
      $segments[] = Playoff::urlsafeB64Encode(json_encode($payload));
      $signing_input = implode('.', $segments);

      $signature = hash_hmac('SHA256', $signing_input, $params['client_secret'], true);
      $segments[] = Playoff::urlsafeB64Encode($signature);
